...
pub struct Bitcoind {
    ...
    pub(crate) wallet_client: Arc<JitTry<Arc<bitcoincore_rpc::Client>>>,
...
impl Bitcoind {
    pub async fn new(processmgr: &ProcessManager) -> Result<Self> {
        ...
        let wallet_client = JitTry::new(|| async move {
            let client =
...
impl Lightningd {
    pub async fn new(process_mgr: &ProcessManager, bitcoind: Bitcoind) -> Result<Self> {
        ...
        // workaround: clightnig will crash on start if it gets a bad reasponse from
        // bitcoind
        ...
        poll("lightningd", Duration::from_secs(30), || async {
...
impl Lnd {
    pub async fn new(process_mgr: &ProcessManager, bitcoind: Bitcoind) -> Result<Self> {
        // workaround: lnd will crash on start if it gets a bad reasponse from bitcoind
        bitcoind.poll_ready().await?;
        ...
        poll("lnd_startup", Duration::from_secs(30), || async {
