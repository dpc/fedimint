...
impl ServerModule for Meta {
    ...
    async fn consensus_proposal(
        ...
        for (key, value) in desired {
            if Self::is_submitted_value_redundant(dbtx, key, &value, self.our_peer_id).await {
                to_delete.push(key);
            } else {
                to_submit.push(MetaConsensusItem { key, value });
            }
        }
    ...
    async fn process_consensus_item<'a, 'b>(
        ...
        let matching_submissions: Vec<PeerId> = dbtx
            ...
            .await
            .filter(|(_submission_key, submission_value)| future::ready(&value == submission_value))
