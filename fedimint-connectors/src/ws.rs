...
impl WebsocketConnector {
    ...
    ///
    /// This provides a narrow destination guarantee only when the caller
    /// separately validates and passes the exact single-address URL: `ws` with
    /// an IP literal, or `wss` with an IPv4 literal. Disabling redirects then
    /// dials the supplied endpoint once and does not connect to a redirect
    /// target. jsonrpsee still parses an absolute `Location` and may resolve
    /// its hostname before returning.
    ///
    ...
    /// Redirect rejection therefore does not provide a destination guarantee
    /// for hostname `ws` or `wss` URLs. IPv6-literal `wss` is unsupported by
    /// jsonrpsee 0.24.9. This setting does not provide SafeUrl or address
    /// policy, DNS/TCP pinning, TLS/SNI control, or proxy/routing control.
    #[cfg(not(target_family = "wasm"))]
